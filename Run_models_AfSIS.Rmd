...
parameters.MCMC <- read.csv("parameters/Century_MCMC_out_SingleBest_112525.csv") #MCMC fitted
parameters.file.test <- parameters.file %>% mutate(V2 = case_when(
...
Century_ss_AllSites <- forcing_data_Century %>%
  ...
  mutate(Soil_Organic_Carbon_kg_m2 = (StrLitter + MetLitter + ACTIVE + SLOW + PASSIVE) / 1000, Type = "Century")
