...
forcing_data_Century <- forcing_data %>% 
  #does this needs to be per month to match monthly timestep of the model?
  mutate(npp_modis.gC.m2.month = npp_modis*1000/12, #Century units are mass/area/time
         silt = (Clay_63um - Clay_2um)/100,  #AfSIS % to fraction in Century
         clay = Clay_2um/100,  #AfSIS % to fraction in Century
         surfaceIn = npp_modis.gC.m2.month/2, #Assume 50/50 split
         soilIn = npp_modis.gC.m2.month/2, 
         # is LIG_N a ratio or percent?; if ratio no need to divide by 100
         LN = LIG_N/100, #percent in AfSIS to fraction in Century
         Ls = LIG/100) %>% #percent in AfSIS to fraction in Century
  drop_na(npp_modis.gC.m2.month, pH, Clay_2um, LN, stemp, sm, Clay_63um, bd_extracted) %>%
  dplyr::select(SSN, PET, MAP, MAT, LN, clay, silt, Ls, surfaceIn, soilIn, Depth)
```

#Run Century at AfSIS sites
```{r}
#run for 1000 years
mnths = seq(0,12*1000)

results <- lapply(seq_len(nrow(forcing_data_Century)), function(i) {
  row <- forcing_data_Century[i, ]

# --- Step 1: Compute environmental modifiers ---
  # Use your temperature and moisture columns here (adjust names as needed)
  fT <- fT.Century1(row$MAT)       # Air temperature in degrees C
  fW <- fW.Century(row$MAP/12, row$PET/12) # PPT monthly precip, PET = monthly PET, both in mm
  xi_val <- fT * fW                   # combine multiplicatively

  # If your t (mnths) is a time vector, make xi a data frame with constant values
  xi_df <- data.frame(time = mnths, xi = rep(xi_val, length(mnths)))

  # --- Step 2: Run the Century model ---
  cm <- CenturyModel(
    t = mnths,
    surfaceIn = row$surfaceIn,
    soilIn = row$soilIn,
    #should this also be row$LN and row$Ls?
    LN = row$LN,
    Ls = row$Ls,
    clay = row$clay,
    silt = row$silt,
    xi = xi_df       # include environmental forcing here
  )

  # --- Step 3: Extract last carbon pool values ---
  Ct <- getC(cm)
  last_row <- Ct[nrow(Ct), , drop = FALSE]
...
poolNames <- c("SurfaceStructural", "SurfaceMetabolic", "BelowgroundStructural",
               "BelowgroundMetabolic", "ActiveSOM", "SlowSOM", "PassiveSOM")

final_results <- as.data.frame(final_results)

names(final_results) <- poolNames

#Merge model results with original site parameters; only consider the three main pools
CENTURY_ss_AllSites <- as.data.frame(cbind(forcing_data_Century, final_results)) %>% 
  mutate(Soil_Organic_Carbon_kg_m2 = (ActiveSOM + SlowSOM + PassiveSOM) / 1000, 
         Type = "Century")

# write.csv(CENTURY_ss_AllSites, "./model_output/Century_results.csv")
```

#Century without temp and PPT
```{r}
...
#   Ct <- getC(cm)
# 
#   # Keep only the last row of the matrix
#   last_row <- Ct[nrow(Ct), , drop = FALSE]
# 
#   return(last_row)
# })
...
# CENTURY_ss_AllSites <- as.data.frame(cbind(forcing_data_Century,final_results))
# 
# write.csv(CENTURY_ss_AllSites, "./model_output/Century_results_without_scalars.csv")
```
