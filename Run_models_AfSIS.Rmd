...
forcing_data_MIMICS <- forcing_data %>% 
  dplyr::mutate(npp_modis.gC.m2.yr = npp_modis*1000) %>%
  drop_na(npp_modis.gC.m2.yr, pH, Clay_2um, LIG_N, stemp, sm, Clay_63um, bd_extracted,
          Litterfall.gC.m2.yr) %>%
  dplyr::select(Set, SSN, ANPP = npp_modis.gC.m2.yr, 
                CLAY = Clay_2um,
                lig_N = LIG_N,
                TSOI = stemp,
                theta_liq = sm,
...
forcing_data_Millennial <- forcing_data %>% 
  dplyr::mutate(npp_modis.gC.m2.d = npp_modis*1000/365) %>%
  ...
  dplyr::select(Set, param_pH = pH, 
                ...
                SSN = SSN,
                Depth = Depth) %>% 
  dplyr::mutate(param_bulkd = param_bulkd*1000*0.2, 
...
parameters.MCMC <- read.csv("parameters/Century_MCMC_out_SingleBest_120325.csv") #MCMC fitted
parameters.file <- parameters.file %>% mutate(V2 = case_when(
...
MCMC_SingleBest <- read.csv("parameters/AfSIS_MIMICS_MCMC_Parameterized.csv")
