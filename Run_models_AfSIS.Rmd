...
source(here("CenturyFunction/CenturyFunction_mod.R"))
```
...
forcing_data_Century <- forcing_data %>% 
  mutate(npp_modis.gC.m2.week = npp_modis*1000/52, #Century units are mass/area/time
         silt = (Clay_63um - Clay_2um)/100,  #AfSIS % to fraction in Century
         clay = Clay_2um/100,  #AfSIS % to fraction in Century
         surfaceIn = npp_modis.gC.m2.week/2, #Assume 50/50 split
         soilIn = npp_modis.gC.m2.week/2, 
         # is LIG_N a ratio or percent?; if ratio no need to divide by 100
         LN = LIG_N, #percent in AfSIS to fraction in Century
         Ls = LIG/100) %>% #percent in AfSIS to fraction in Century
  drop_na(npp_modis.gC.m2.week, pH, Clay_2um, LN, stemp, sm, Clay_63um, bd_extracted) %>%
  dplyr::select(SSN, Set, PET, MAP, MAT, LN, clay, silt, Ls, surfaceIn, soilIn, Depth)
```
...
# --- Step 1: Compute environmental modifiers ---
  # Use default century temperature and moisture modifiers
  ...
  # If your t (time) is a time vector, make xi a data frame with constant values
  xi_df <- data.frame(time = time, xi = rep(xi_val, length(mnths)))
