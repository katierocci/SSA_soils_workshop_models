...
forcing_data_Century <- forcing_data %>%
  ...
  dplyr::select(SSN_row_ID, SSN, Set, forc_st, forc_sw, forc_npp, LN, param_claysilt, 
                LigFrac, Soil_Organic_Carbon_kg_m2, input_to_metlitter)
...
# parameters.file <- read.table(here("parameters/parameter_fit_SSA_Century.csv")) #fitted
...
run_century_for_site <- function(site_row, base_params){
...
results_list <- apply(forcing_data_Century, 1, function(row) {
  ...
  run_century_for_site(row, parameters)
})
...
# parameters.file <- read.table(here("parameters/parameter_fit_SSA_update.csv")) #fitted
parameters <- as.list(parameters.file$V2)
names(parameters) <- parameters.file$V1
