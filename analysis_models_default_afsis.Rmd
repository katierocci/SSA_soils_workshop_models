...
```{r}
model_out <- read.csv("./model_output/both_model_results_default_run.csv") %>% 
  #rename SOC column
  dplyr::rename(C_stock_pred_kg_m2 = Soil_Organic_Carbon_kg_m2)

afsis_data <- read.csv("forcing_data/afsis_ref_updated6.csv", as.is=T) %>% 
  dplyr::select(SSN, Site, Cluster, Plot, Country, Longitude, Latitude,
                Clay_2um, Clay_8um, Clay_63um, LIG_N, SoilTMP_C, SoilMoi_m3m3,
                Depth, CORG, pH, bd_extracted, Litterfall.gC.m2.yr, NPP.gC.m2.d,
                Am_Ox_Al, Am_Ox_Fe, Caex, Mgex, Naex, Clay_1_1, Clay_2_1) %>% 
...
data_merged %>%
  ...
  scale_x_continuous("Predicted SOC (kg/m2)", expand = c(0,0), limits = c(0,20)) +
  scale_y_continuous("Observed SOC (kg/m2)", expand = c(0,0), limits = c(0,25)) +
  geom_abline(slope = 1) +
...
```{r}
#MIMICS
p1 <- plot_fun_soil_prop(model = "MIMICS", xvar = Litterfall.gC.m2.yr)
p2 <- plot_fun_soil_prop(model = "MIMICS", xvar = Clay_2um)
p3 <- plot_fun_soil_prop(model = "MIMICS", xvar = LIG_N)
p4 <- plot_fun_soil_prop(model = "MIMICS", xvar = SoilTMP_C)
p5 <- plot_fun_soil_prop(model = "MIMICS", xvar = SoilMoi_m3m3)
...
p4 <- plot_fun_soil_prop(model = "Millennial", xvar = SoilTMP_C)
p5 <- plot_fun_soil_prop(model = "Millennial", xvar = SoilMoi_m3m3)
p6 <- plot_fun_soil_prop(model = "Millennial", xvar = NPP.gC.m2.d)
