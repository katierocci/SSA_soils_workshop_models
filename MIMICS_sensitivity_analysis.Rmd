...
forcing_data_MIMICS <- forcing_data %>% 
  ...
  drop_na(npp_modis.gC.m2.yr, pH, Clay_2um, LIG_N, stemp, sm, Clay_63um, 
          bd_extracted) %>%
  select(ANPP = npp_modis.gC.m2.yr, 
         CLAY = Clay_2um,
         lig_N = LIG_N,
         TSOI = stemp,
         theta_liq = sm)
...
write.csv(MIMICS_ss_AllSites, row.names = FALSE,
          paste0("./model_output/MIMICS_SensitivityAnalysisOutput_",
                 Sys.Date(), ".csv"))
```
